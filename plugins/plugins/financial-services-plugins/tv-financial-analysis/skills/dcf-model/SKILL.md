---
name: dcf-model
description: Real DCF (Discounted Cash Flow) model creation for equity valuation using TradingView MCP data. Retrieves financial data from TradingView MCP server, builds comprehensive cash flow projections with proper WACC calculations, performs sensitivity analysis, and outputs professional Excel models with executive summaries. Use when users need to value a company using DCF methodology with TradingView as the primary data source.
---

# DCF Model Builder (TradingView Edition)

## Overview

This skill creates institutional-quality DCF models for equity valuation following investment banking standards, using TradingView MCP as the sole data source. Each analysis produces a detailed Excel model (with sensitivity analysis included at the bottom of the DCF sheet).

## Tools

- TradingView MCP (get_fundamentals, get_credit_ratings, get_quote, get_financial_statements)
...
Fetch data from TradingView MCP server.

**TradingView MCP Tools:**

1. **get_fundamentals(symbol)** — Financial metrics (revenue, EBITDA, CapEx, depreciation, beta, market cap, net debt, tax rate components)
2. **get_credit_ratings(symbol)** — Credit ratings for cost of debt estimation
3. **get_quote(symbol)** — Current stock price, shares outstanding, market cap
4. **get_financial_statements(symbol, statement, period)** — Full financial statements if needed

**Key TradingView Fields for DCF:**

| Required Field | TradingView Source | Coverage |
|---------------|-------------------|----------|
| Revenue history | `total_revenue_fq_h` | 100% |
| EBITDA | `ebitda_fq` | 100% |
| CapEx | `capital_expenditures_fq_h` | 100% |
| Depreciation | `depreciation_fy_h` | 100% |
| Beta | `beta_3_year` | 100% |
| Market Cap | `market_cap_basic` | 100% |
| Net Debt | `net_debt_fq` | 100% |
| Tax Rate | Calculate from `income_tax_fq` / `pretax_income_fq` | 100% |
| Credit Rating | `get_credit_ratings` | 90% |
| Current Price | `get_quote` → `price` | 100% |
| Shares Outstanding | `get_quote` → `volume` (or calculate from market cap/price) | 100% |
| Buyback Yield | `buyback_yield` | 85% |
| Share Buyback Ratio (FY) | `share_buyback_ratio_fy` | 85% |
| Total Shares Outstanding | `total_shares_outstanding` | 100% |
...
credit_ratings = mcp_call("get_credit_ratings", {"symbol": symbol})

# Step 3: Fetch current quote
quote = mcp_call("get_quote", {"symbol": symbol})

# Step 4: Calculate derived metrics
tax_rate = fundamentals.get("income_tax_fq", 0) / fundamentals.get("pretax_income_fq", 1)
shares_outstanding = quote.get("market_cap", 0) / quote.get("price", 1)
```

**Validation Checklist:**
- Verify net debt vs net cash (critical for valuation)
- Confirm diluted shares outstanding (check for recent buybacks/issuances — if `buyback_yield` > 0, use `total_shares_outstanding` as buyback-adjusted share count for projections)
...
- **Normalized ROE**: Adjusted ROE accounting for buyback impact using formula: Normalized ROE = net_income_fy / (total_equity_fq + market_cap × buyback_yield/100)
...
Where:
- Risk-Free Rate = Current 10-Year Treasury Yield (USER INPUT REQUIRED)
- Beta = 3-year beta from TradingView `beta_3_year`
- Equity Risk Premium = 5.0-6.0% (USER INPUT REQUIRED)
```
...
- Credit rating from `get_credit_ratings` (if available)
- Interest expense / Total Debt from financials
- Industry benchmarks if data unavailable
```
...
### Buyback Impact on Valuation

Stock buybacks reduce shares outstanding, increasing EPS and per-share metrics. When modeling buybacks in DCF:

**Data Source**: Use `buyback_yield` from TradingView (annualized buyback yield as % of market cap)

**Implementation Steps**:
1. Apply buyback_yield to projected shares: Shares_Year_N = Shares_Year_(N-1) × (1 - buyback_yield/100)
2. Calculate diluted EPS impact: EPS increases as shares decrease
3. Adjust equity value bridge: If modeling buybacks as cash use, reduce net cash position by buyback amount

**Normalized ROE Calculation**:
For historical analysis, adjust ROE to account for buyback-induced equity reduction:
```
Normalized ROE = net_income_fy / (total_equity_fq + market_cap × buyback_yield/100)
```

This captures the true return on capital employed, isolating the effect of financial engineering from operating performance.

## User Prompts for Missing Data

TradingView provides 92% coverage for DCF modeling. The following inputs require user prompts:

### Missing Data: Risk-Free Rate
...
### Missing Data: Market Risk Premium
...
### Missing Data: Terminal Growth Rate
...
### Missing Data: Cost of Debt (if no credit rating)

```markdown
## ⚠️ Data Gap: Cost of Debt

No credit rating available from TradingView for this company.

**Options:**
1. **Use industry average**: ~4.5-6.0% for [sector]
2. **Calculate from interest expense**: [if data available]
3. **Enter custom rate**
...
```csv
Item,Source Comment
Stock price,Source: TradingView get_quote, 2025-10-12 Close price
Shares outstanding,Source: TradingView get_quote, 2025-10-12 Calculated from market cap/price
Historical revenue,Source: TradingView get_fundamentals total_revenue_fq_h
Beta,Source: TradingView get_fundamentals beta_3_year
Credit rating,Source: TradingView get_credit_ratings
...
1. **Company identifier**: Ticker symbol (e.g., "NASDAQ:AAPL", "NYSE:MSFT")
2. **Growth assumptions**: Revenue growth rates for projection period (or "use consensus")
3. **User-provided WACC inputs**:
   - Risk-free rate (10Y Treasury yield)
   - Market risk premium (5-6% standard)
...
Beta (3Y from TradingView),X.XX,[Yellow input]
Equity Risk Premium,X.XX%,[Yellow input]
Cost of Equity,X.XX%,[Calculated blue]
,,
COST OF DEBT CALCULATION,,
Credit Rating,[From TradingView],[Yellow input]
...
1. **Gather market data from TradingView:**
   - Call `get_quote(symbol)` for current stock price
   - Call `get_fundamentals(symbol)` for financial metrics
   - Call `get_credit_ratings(symbol)` for cost of debt input
   - Prompt user for risk-free rate, market risk premium, terminal growth

2. **Gather historical financials:**
   - Extract from `get_fundamentals` → `total_revenue_fq_h`, `ebitda_fq`, etc.
   - Calculate derived metrics (tax rate, shares outstanding)
   - Validate data quality before proceeding

3. **Prompt for missing WACC inputs:**
   - Risk-free rate (10Y Treasury yield)
   - Market risk premium (5-6% standard)
   - Terminal growth rate (2-3% typical)

4. **Begin model construction** using the DCF methodology detailed in this skill
...
- **TradingView MCP** (primary): `get_fundamentals`, `get_credit_ratings`, `get_quote`, `get_financial_statements`
- **User-provided data**: Risk-free rate, market risk premium, terminal growth assumptions
- **Web search/fetch**: Fallback for any missing TradingView data only if explicitly needed
...
- Font colors: Blue=inputs, Black=formulas, Green=sheet links
- Cell comments on ALL hardcoded inputs (including TradingView sources)
