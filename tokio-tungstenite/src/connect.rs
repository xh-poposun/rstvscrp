...
#[cfg(feature = "proxy")]
use crate::proxy::connect_proxy_stream;
...
async fn connect(
    ...
    // Try to connect through proxy if proxy feature is enabled
    #[cfg(feature = "proxy")]
    let socket = {
        if let Some(proxy_stream) = connect_proxy_stream(request.uri(), &domain, port).await? {
            proxy_stream
        } else {
